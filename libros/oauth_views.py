...
def google_oauth_callback(request):
    ...
    if not code:
        ...
        if request.method == 'GET':
            return redirect(f'/oauth/login/?error={quote(error_msg)}')
    ...
    try:
        ...
        redirect_url = (
            ...
            f'user_info={quote(user_info_json)}&'
            f'google_info={quote(google_info_json)}&'
            f'message={quote("Login exitoso con Google" if not created else "Cuenta creada exitosamente con Google")}'
        )
        ...
        return redirect(f'/oauth/login/?error={quote("Timeout al comunicarse con Google")}')
        ...
        return redirect(f'/oauth/login/?error={quote(f"Error inesperado: {str(e)}")}')
